// 1. Elemen DOM Utama
                ...
                let batchesHtml = item.batches.sort((a,b) => new Date(a.expDate) - new Date(b.expDate)).map(b => `
                    <div class="batch-row">
                        <div style="flex: 1; display:flex; justify-content:space-between; align-items:center;">
                            ...
                            <strong>${b.kuantitas} ${item.satuan}</strong>
                ...
                card.innerHTML = `
                    <div class="stock-info">
                        ...
                        <div class="batch-list-container">
                            <div class="batch-list-title">Rincian Batch (FIFO):</div>
                            ${batchesHtml}
                        </div>
                    </div>
                    ...
                    <div class="stock-controls-wrapper">
                        <div class="stock-qty-adjust">
                            <button class="btn-qty" onclick="openIssueModal(${item.id})" title="Keluarkan Stok (FIFO)">-</button>
                            <div class="qty-display">${totalQty} <small>${item.satuan}</small></div>
                            <button class="btn-qty" onclick="openAddBatchModal(${item.id})" title="Tambah Stok">+</button>
                        </div>
        ...
        // 7. Modifikasi Form Keluarkan Stok
...
` +
                    ...
                    historyContainer.innerHTML += `
                        <div class="history-item">
                            <div class="history-header">
                                ...
                                <span class="badge ${isMasuk ? 'success' : 'danger'}">${log.jenis}</span>
                            </div>
                            <div class="history-meta">
                                <span>${log.tanggal}</span>
                                <strong class="${colorClass}">${sign}${log.jumlah} ${log.satuan}</strong>
                            </div>
                            <div style="font-size: 0.85rem; margin-top: 5px; color: var(--text-muted);">
                                Sisa stok akhir: <strong style="color: var(--text-color);">${log.sisa} ${log.satuan}</strong>
                            </div>
                            ${extraInfoHtml}
                        </div>`;
                });
        ...
        formEditStok.addEventListener('submit', function(e) { 
            ...
            if (item) { 
                ...
                item.satuan = document.getElementById('editSatuan').value; 
        ...
        dashCards.forEach(card => { 
            card.addEventListener('click', () => { 
                ...
                currentAlertFilter = card.getAttribute('data-alert'); 
                renderStockData(); 
            }); 
        });
